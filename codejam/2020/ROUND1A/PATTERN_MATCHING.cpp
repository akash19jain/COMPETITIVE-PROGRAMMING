...
{ 
    ...
    CASES
    {
    	...
    	REP(i,n)
    	{
    		...
    		for(k=str[i].size();str[i][k]!='*';k--)
    		{
    			if(str[i].size()-1-k<suf.size() && suf[str[i].size()-1-k]!=str[i][k])
    			{
    				fail=true;
    				break;
    			}
    			if(str[i].size()-1-k == suf.size())
    				suf+=str[i][k];
    		}
