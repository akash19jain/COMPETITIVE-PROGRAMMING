...
int main()
{
    ...
    FOR(tt,1,t)
    {
    	...
    	REP(i,n)
    	{
    		ll x,y;
    		SC2(x,y);
    		arr.push_back(make_pair(x,make_pair(y,i)));
    	}
    	...
    	ll c0=arr[0].second.first,c1=0;
    	int f=0;
    	char ans[n+5],a1=0;
    	ll z=arr[0].second.second;
    	ans[z]='J';
    	REP(i,n-1)
    	{
    		if(c0<=arr[i+1].first)
    		{
    			ll z=arr[i+1].second.second;
    			ans[z]='J';
    			c0=arr[i+1].second.first;
    		}
    		else if(c1<=arr[i+1].first)
    		{
    			ll z=arr[i+1].second.second;
    			ans[z]='C';
    			c1=arr[i+1].second.first;
    		}
    		else
    		{
    			f=1;
