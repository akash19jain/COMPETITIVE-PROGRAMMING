...
ll dsufind(ll v)
{
	if (parent[v] == v)
		return v;
	parent[v] = dsufind(parent[v]);
	return parent[v];
}

void dsuunion(ll a, ll b)
{
	ll u = dsufind(a);
	ll v = dsufind(b);
...
void solve(ll n, ll g, ll q, ll origin[], ll dest[])
{
	REP(i, n + 1)
	{
		make_set(i);
	...
	REP(i, q)
	{
		if (dsufind(origin[i]) == dsufind(dest[i]))
		{
			PFS("1");
		}
		else
