...
public class I {
	public static void main(String args[])throws IOException {
		...
		int t = io.ni();
        int n, k, posSize, negSize, min, sum, a;
        int[] pos, neg;
		...
		while(t-->0) {
            ...
            for(int i=0;i<n;i++) {
                a = io.ni();
                if(a>=0) pos[posSize++] = a;
                else neg[negSize++] = a;
            }
            
            min = Meth.min(negSize, k);
            Arrays.sort(neg, 0, negSize);
            for(int i=0;i<min;i++) neg[i] *= -1;
            k -= min;

            if(k>0 && k%2==1) {
                if(negSize>0 && posSize>0) {
                    Arrays.sort(pos, 0, posSize);
                    Arrays.sort(neg, 0, negSize);
                    if(pos[0]<neg[0]) pos[0] *= -1;
                    else neg[0] *= -1;
                } else if(posSize>0) {
					Arrays.sort(pos, 0, posSize);
					pos[0] *= -1;
            ...
            sum = 0;
            for(int i=0;i<posSize;i++) sum += pos[i];
            for(int i=0;i<negSize;i++) sum += neg[i];
