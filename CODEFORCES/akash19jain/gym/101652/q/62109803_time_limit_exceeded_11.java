...
public class Q {
	public static void main(String args[])throws IOException {
		...
		long n = io.nl();
        long midCmp = ((n*(n-1))/2) / 2;
        // long res = bsearch_floor(midCmp, 1, n);  
		long res = linear_search(midCmp, n);     

		System.out.print(res);
	}

	static long linear_search(long key, long n) {
		for(int i=1;i<=n;i++) {
			long val = get(i,n);
			if(val>=key) return i;
		}
  ...
  static long bsearch_floor(long key, long low, long high) {
    long mid, n=high, val;
    ...
    if(get(low,n)>=key) return low;
    return high;
  }

  static long get(long i, long n) { 
    return (i * (n-1 + n-i) ) / 2L ;
  }
}
