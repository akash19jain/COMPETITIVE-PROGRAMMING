...
public class N {
	public static void main(String args[])throws IOException {
        ...
        for(int len=2;len<=n && odd;len++) {
            for(int start=0, end=start+len-1;end<n && odd;start++, end++) {
                if(len==2) dp[start][end] = s[start]==s[end];
                else dp[start][end] = dp[start+1][end-1] && (s[start]==s[end]);
                if(dp[start][end] && len%2==0) odd=false;
            }
        }

		System.out.print(odd ? "Odd." : "Or not.");
	}
