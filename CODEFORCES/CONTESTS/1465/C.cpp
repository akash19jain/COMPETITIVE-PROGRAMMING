...
bool dfs(vector<ll> adj[], ll u, vector<bool>& visited, vector<bool>& rec)
{
	if (!visited[u])
	{
		visited[u] = true;
...
int main()
{
	...
	CASES
	{
		...
		vector<bool> visited(n + 1, false), rec(n + 1, false);
		FOR(i, 1, n)
		{
			if (!visited[i] && dfs(adj, i, visited, rec))
			{
				ans++;
			}
		}
