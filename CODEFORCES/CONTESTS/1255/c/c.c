...
int main()
{
    ...
    //finding the second and third element
    x=a[ans[0]-1][0][0];
    y=a[ans[0]-1][0][1];
    if(count[x-1]==2)
    {
    ...
    FOR(i,3,n-1)
    {
        REP(j,3)
        {
            if(a[ans[i-1]-1][j][0]==ans[i-2] && a[ans[i-1]-1][j][1]!=ans[i-3])
            {
                ans[i]=a[ans[i-1]-1][j][1];
                break;
            }
            if(a[ans[i-1]-1][j][1]==ans[i-2] && a[ans[i-1]-1][j][0]!=ans[i-3])
            {
                ans[i]=a[ans[i-1]-1][j][0];
                break;
            }
        }
    }
