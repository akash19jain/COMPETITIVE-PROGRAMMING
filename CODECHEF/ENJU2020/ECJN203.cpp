...
bool sortcmp(const pair<ll, pair<ll, ll> > &a, const pair<ll, pair<ll, ll> > &b)
{
	if (a.first != b.first)
...
int main()
{
	...
	REP(i, n)
	{
		ll x, y;
		SC2(x, y);
		arr.push_back(make_pair(y, make_pair(x, i + 1)));
	}
	sort(arr.begin(), arr.end(), sortcmp);
	REP(i, n)
	{
		printf("%lld ", arr[i].second.second);
	}
