...
ll solve(ll parent[], ll v, ll w, ll n)
{
	if (((2 * v > n) || (parent[2 * v] == 0)) && (((2 * v) + 1 > n) || (parent[(2 * v) + 1] == 0)))
	{
		return w;
	}

	ll childcount = 0;
	if (((2 * v <= n) || (parent[2 * v] > 0)))
		childcount++;

	if ((((2 * v) + 1 <= n) || (parent[(2 * v) + 1] > 0)))
		childcount++;
	...
	if (w % childcount == 0)
	{
		...
		{
			c++;
			sum += solve(parent, (2 * v) + 1, w / childcount, n);
		}
		if ((2 * v) <= n)
		{
			c++;
			sum += solve(parent, 2 * v, w / childcount, n);
		}
...
int main()
{
	...
	while (q--)
	{
		ll ans = solve(parent, arr[i][0], arr[i][1], n);
