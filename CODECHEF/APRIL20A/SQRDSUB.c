...
ll solve(ll arr[],ll n,ll p,ll v)
{

	if(p>=n)
	{
		if(arr[p]%2==1 || arr[p]%4==0)
			return 1;
		return 0;
	}
	if(dp[p][v]!=-1)
	...
	ll c=(arr[p]%2==1 || arr[p]%4==0);
	
	// DEB(p);
	// DEB(v);
	// DEB(x);
	DEB(c);
	return dp[p][v]=c+solve(arr,n,p+1,(v*arr[p])%4);
