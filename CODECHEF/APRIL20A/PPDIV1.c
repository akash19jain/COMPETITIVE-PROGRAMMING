//IDEA TAKEN FROM : http://oeis.org/A091051
//TESTING IF IT WORKS FOR LARGE INPUTS
/*  AUTHOR: AKASH JAIN
*   EMAIL:  akash19jain@gmail.com
*   ID:     akash19jain  
*   DATE:   07-04-2020 23:24:16
*/
...
int main()
{
    ...
    #endif 
    ll max=1e12+5;
    set<ll> perfect_power;
    perfect_power.insert(1);
    FOR(i,2,log2(max)+1)
    {
    	ll x=2;
    	while(pow(x,i)<=max)
    	{
    		perfect_power.insert(pow(x,i));
    		x++;
    	}
    }
    ...
    for(std::set<ll>::iterator it=perfect_power.begin();it!=perfect_power.end();++it)
    	cout<<*it<<",";
    //     ll t;
    //     SC1(t);
