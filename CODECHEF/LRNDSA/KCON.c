...
int main()
{
	...
	CASES
	{
		...
		SCA(arr, n);
		ll su = 0;
		ll sum = 0, ans = *max_element(arr, arr + n);
		...
		if (k > 1)
		{
			ll prefix = 0, suffix = 0, p = 0, s = 0;
			REP(i, n)
			{
				p += arr[i];
				s += arr[n - 1 - i];
				prefix = maxv(prefix, p);
				suffix = maxv(suffix, s);
			}
			if (prefix + suffix > 0)
			{
				long long sum = max(accumulate(arr, arr + n, 0ll), 0ll);
				ans = maxv(ans, prefix + suffix + (k - 2) * sum);
			}
		}
		PF1(ans);
	}
