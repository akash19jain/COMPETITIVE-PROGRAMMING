...
int main()
{
	...
	CASES
	{
		...
		REP(i, n)
		{
			SC1(x);
			if (x == 0)
				defence.push_back(price[i]);
			else
				forward.push_back(price[i]);
		}

		sort(defence.begin(), defence.end());
		sort(forward.begin(), forward.end());

		if (defence.size() == 0 || forward.size() == 0)
		{
			PFS("no");
		}
		else if (defence[0] + forward[0] <= rem)
		{
			PFS("yes");
		}
		else
